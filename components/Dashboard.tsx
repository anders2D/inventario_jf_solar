...
import { LayoutDashboard, AlertCircle, TrendingUp, TrendingDown, PackageCheck, ChevronRight, Activity, Zap, ArrowDownCircle, ArrowUpCircle, Settings } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ inventory, transactions, onNavigate }) => {
  ...
  const handleSaveWidgets = (widgets: typeof visibleWidgets) => {
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Resumen general del almacén</p>
        </div>
        <button
          onClick={() => setShowCustomize(true)}
          className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-2xl font-bold hover:bg-blue-600 hover:text-white transition-all shadow-sm"
        >
          <Settings size={18} />
          Personalizar
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {visibleWidgets.alerts && (
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200/40 dark:border-slate-800/40 shadow-sm relative overflow-hidden animate-in fade-in slide-in-from-left-4 duration-500">
        ...
        {visibleWidgets.activity && (
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200/40 dark:border-slate-800/40 shadow-sm relative overflow-hidden animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="absolute top-0 right-0 p-8 opacity-5">
